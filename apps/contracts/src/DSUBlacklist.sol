...
contract DSUBlacklist is IDSUBlacklist, AccessControl {
    ...
    function batchBlacklist(address[] memory accounts, bool toBeBlacklisted) public onlyRole(BLACKLIST_ROLE) {
        for (uint256 i; i < accounts.length; ++i) {
            toBeBlacklisted ? _addToBlacklist(accounts[i]) : _removeFromBlacklist(accounts[i]);
        }
    }
