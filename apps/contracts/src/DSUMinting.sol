...
import {IDSU} from "./interfaces/IDSU.sol";
import {IDSUMinting} from "./interfaces/IDSUMinting.sol";
import {IWETH9} from "./interfaces/IWETH9.sol";

/**
 * @title DSU Minting
 * @notice This contract mints and redeems DSU, the first staked Ethereum delta-neutral backed synthetic dollar
 */
contract DSUMinting is IDSUMinting, SingleAdminAccessControl, ReentrancyGuard {
    ...
    bytes32 private constant ORDER_TYPE = keccak256(
        "Order(string order_id,uint8 order_type,uint128 expiry,uint120 nonce,address benefactor,address beneficiary,address collateral_asset,uint128 collateral_amount,uint128 dsu_amount)"
    );
    ...
    /// @notice EIP712 name
    bytes32 private constant EIP_712_NAME = keccak256("DSUMinting");
    ...
    /// @notice dsu stablecoin
    IDSU public immutable dsu;
    ...
    /// @notice running total DSU minted/redeemed per single block
    mapping(uint256 => BlockTotals) public totalPerBlock;

    /// @notice total DSU that can be minted/redeemed across all assets per single block.
    ...
    /// @notice ensure that the already minted DSU in the actual block plus the amount to be minted is below the maximum mint amount
    /// @param mintAmount The DSU amount to be minted
    ...
    /// @notice ensure that the already redeemed DSU in the actual block plus the amount to be redeemed is below the maximum redeem amount
    /// @param redeemAmount The DSU amount to be redeemed
    ...
    /// @notice ensure that the global, overall minted DSU in the actual block
    ...
    /// @notice ensure that the global, overall redeemed DSU in the actual block
    ...
    constructor(
        ...
        if (address(_dsu) == address(0)) revert InvalidDSUAddress();
        ...
        // Set the global max DSU mint/redeem limits
        globalConfig = _globalConfig;

        // Set the max mint/redeem limits per block for each asset
        for (uint128 k = 0; k < _tokenConfig.length;) {
            if (tokenConfig[_assets[k]].isActive || _assets[k] == address(0) || _assets[k] == address(dsu)) {
                revert InvalidAssetAddress();
            }
    ...
    function mint(Order calldata order, Route calldata route, Signature calldata signature)
        ...
        belowMaxMintPerBlock(order.dsu_amount, order.collateral_asset)
        belowGlobalMaxMintPerBlock(order.dsu_amount)
    {
        ...
        totalPerBlockPerAsset[block.number][order.collateral_asset].mintedPerBlock += order.dsu_amount;
        totalPerBlock[block.number].mintedPerBlock += order.dsu_amount;
        _transferCollateral(
            order.collateral_amount, order.collateral_asset, order.benefactor, route.addresses, route.ratios
        );
        dsu.mint(order.beneficiary, order.dsu_amount);
        emit Mint(
            order.order_id,
            order.benefactor,
            order.beneficiary,
    ...
    function redeem(Order calldata order, Signature calldata signature)
        ...
        belowMaxRedeemPerBlock(order.dsu_amount, order.collateral_asset)
        belowGlobalMaxRedeemPerBlock(order.dsu_amount)
    {
        if (order.order_type != OrderType.REDEEM) revert InvalidOrder();
        verifyOrder(order, signature);
        _deduplicateOrder(order.benefactor, order.nonce);
        // Add to the redeemed amount in this block
        totalPerBlockPerAsset[block.number][order.collateral_asset].redeemedPerBlock += order.dsu_amount;
        totalPerBlock[block.number].redeemedPerBlock += order.dsu_amount;
        dsu.burnFrom(order.benefactor, order.dsu_amount);
    ...
    /// @notice Sets the overall, global maximum DSU mint size per block
    ...
    /// @notice Sets the overall, global maximum DSU redeem size per block
    ...
    function addCustodianAddress(address custodian) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (custodian == address(0) || custodian == address(dsu) || !_custodianAddresses.add(custodian)) {
        ...
        returns (bytes32 taker_order_hash)
    {
        ...
        if (typeOfToken == TokenType.STABLE) {
            if (
                !verifyStablesLimit(order.collateral_amount, order.dsu_amount, order.collateral_asset, order.order_type)
            ) {
                revert InvalidStablePrice();
            }
        }
        if (order.beneficiary == address(0)) revert InvalidAddress();
        if (order.collateral_amount == 0 || order.dsu_amount == 0) revert InvalidAmount();
    ...
    function verifyStablesLimit(
        ...
        uint128 dsuDecimals = _getDecimals(address(dsu));
        uint128 collateralDecimals = _getDecimals(collateralAsset);

        uint128 normalizedCollateralAmount;
        uint128 scale = uint128(
            dsuDecimals > collateralDecimals
                ? 10 ** (dsuDecimals - collateralDecimals)
                : 10 ** (collateralDecimals - dsuDecimals)
        );

        normalizedCollateralAmount =
            dsuDecimals > collateralDecimals ? collateralAmount * scale : collateralAmount / scale;

        uint128 difference = normalizedCollateralAmount > dsuAmount
            ? normalizedCollateralAmount - dsuAmount
            : dsuAmount - normalizedCollateralAmount;

        uint128 differenceInBps = (difference * STABLES_RATIO_MULTIPLIER) / dsuAmount;

        if (orderType == OrderType.MINT) {
            return dsuAmount > normalizedCollateralAmount ? differenceInBps <= stablesDeltaLimit : true;
        } else {
            return normalizedCollateralAmount > dsuAmount ? differenceInBps <= stablesDeltaLimit : true;
        }
    }
    ...
    function addSupportedAsset(address asset, TokenConfig memory _tokenConfig) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (tokenConfig[asset].isActive || asset == address(0) || asset == address(dsu)) {
            revert InvalidAssetAddress();
        }
